...
namespace ZzzLab.AspCore.Controllers
{
    ...
    public class DebugLogController : AuthApiControllerBase
    {
        /// <summary>
        /// 로거 목록을 가져온다
        /// </summary>
        ...
        [HttpGet]
        [Route("")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(RestGridResponse<string>))]
        public IActionResult GetLogger()
        {
            return RestResult.Empty<string>();
        }

        /// <summary>
        /// 회사 목록을 가져온다
        /// </summary>
        /// <param name="logerName">로거명</param>
        /// <param name="pageNum">페이지번호</param>
        /// <param name="pageRow">한번에 가져올 수</param>
        /// <returns></returns>
        [HttpGet]
        [Route("{logerName}/List")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(RestGridResponse<dynamic>))]
        public IActionResult GetList(string logerName, uint pageNum = 1, uint pageRow = 20)
            => GetList(logerName, new GridRequest
            {
                PageNum = pageNum,
                PageRow = pageRow
            });
        ...
        ///  회사 목록을 가져온다
        /// </summary>
        /// <param name="logerName"></param>
        /// <param name="req">GridRequest</param>
        /// <returns></returns>

        [HttpPost]
        [Route("{logerName}/List")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(RestGridResponse<LoginInfo>))]
        public IActionResult GetList(string logerName, GridRequest req)
        {
            try
            {
                ...
                using (IDBHandler DB = DataBaseHandler.Create(AppConstant.ConnectionName))
                {
                    ...
                    if (table == null || table.Rows.Count == 0) return RestResult.Grid(Enumerable.Empty<dynamic>(), recordsTotal, recordsFiltered);

                    return RestResult.Grid(table.ToModeling(), recordsTotal, recordsFiltered);
                }
            }
            catch (Exception ex)
            {
