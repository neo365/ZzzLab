...
namespace ZzzLab.Diff.Image
{
    public static class BitmapExtension
    {
        ...
        public static Bitmap ImageDiff(Stream sourceStream, Stream targetStream, Color? maskColor = null, bool overWrite = false)
        {
            ...
            using (Bitmap source = System.Drawing.Image.FromStream(sourceStream) as Bitmap)
            {
                using (Bitmap target = System.Drawing.Image.FromStream(targetStream) as Bitmap)
                {
                    return source.ImageDiff(target, maskColor, overWrite);
                }
            }
        }
        ...
        public static Bitmap ImageDiff(string sourcePath, string targetPath, Color? maskColor = null, bool overWrite = false)
        {
            ...
            using (Bitmap source = System.Drawing.Image.FromFile(sourcePath) as Bitmap)
            {
                using (Bitmap target = System.Drawing.Image.FromFile(targetPath) as Bitmap)
                {
                    return source.ImageDiff(target, maskColor, overWrite);
                }
            }
